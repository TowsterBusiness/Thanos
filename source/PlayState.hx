	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				{
					daBurning = songNotes[3];
				}

				var gottaHitNote:Bool = section.mustHitSection;

				if ((songNotes[1] > 3 && songNotes[1] < 8) || songNotes[1] > 11)
				{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, daBurning, oldNote);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, daBurning, oldNote, true);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'normal':
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets_2');
					babyArrow.animation.addByPrefix('green', 'arrowLEFT0');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN0');
					babyArrow.animation.addByPrefix('purple', 'arrowUP0');
		...
		if (generatedMusic)
			{
				...
				{	
					...
					{
							if (daNote.isSustainNote && daNote.wasGoodHit || daNote.burning > 0)
							{
								daNote.kill();
								notes.remove(daNote, true);
							}
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		{
			...
			note.rating = Ratings.CalculateRating(noteDiff, note.burning, Math.floor((PlayStateChangeables.safeFrames / 60) * 1000));
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if(loadRep)
				{
					...
					note.rating = Ratings.CalculateRating(noteDiff, note.burning);
